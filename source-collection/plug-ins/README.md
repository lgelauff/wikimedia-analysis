...
| [sources-linter](sources-linter/) | [sources-linter.plugin](sources-linter.plugin) | Validates `sources.txt` on every Write/Edit: required fields, citekey convention, duplicate keys, valid access values. |
| [robots-checker](robots-checker/) | [robots-checker.plugin](robots-checker.plugin) | Intercepts `fetch.py --ignore-robots` runs and reports which domains disallow our User-Agent before proceeding. |
