# Atomic-statement criteria + rating
...
scoped / faithful framing) plus the extraction-faithfulness additions already written into #5.
...
This is the **independent-check** step — a *different* agent than the one that extracted (#5),
to avoid self-grading bias.

## Scope
6 wikis; all statements from #5. Current snapshot.

## Inputs
- **#5** statement store (populated).
...
- `statement_criteria.md` — the **already-established** criteria *consolidated* into one versioned
  ...
  come from wiki-polis §3 + #5's faithfulness additions:
...
- A **rater-validation report** (produced *before* the full run): agreement between the rating agent
  ...
  enough to apply.
- Per-statement ratings written back to the store (`ratings` field per #4): each criterion
...
4. Emit the worklist; `revise` items can loop back to #5 for re-extraction.
...
## Dependencies
#5. (Consolidating the criteria + building the rater-validation gold set can start immediately, in parallel with #5.)
