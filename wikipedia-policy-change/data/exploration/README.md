...
summary · reverted? · same-page run · **cross-page series** (same author editing OTHER dataset pages within a window = one coordinated
decision) · **talk-page activity** (author also edited the `Overleg`/talk page = a discussed, deliberate change, with the thread linked)** → HTML (clickable diffs) + CSV. On
...
bulk maintenance series (Romaine "Linkfix", Melsaran "{{vast}}" rename) — so the human resolves decisions, not edits. Signal stack sorts the queue: series/talk = real change (review as a decision), reverted = churn, typo-summary = trivial — leaving only the small **no-signal residue** (isolated unexplained small edits, ~220/962 on the moderator-regulation set) for actual manual attention.
